...
def explain_prediction_local(
    ...
    """Estimate local input-driver shares for one prediction.

    This is a lightweight local sensitivity explanation designed for deployment.
    For each raw input column, the function hides that column by passing it as
    missing, reruns the exact trained pipeline in a vectorized batch, and measures
    how much the predicted log1p(Tau_ms) changes. The top-N absolute changes are
    normalized to 100%.

    Notes
    -----
    - This is not a causal attribution and not SHAP.
    - For numeric fields, hiding means the pipeline falls back to the training
      median through imputation.
    - For categorical fields, hiding means the pipeline uses the Missing category.
    - Effects include engineered features derived from the hidden field. For
      example, hiding Gate_voltage_V also changes gate-pulse-dose features.
    """
    ...
    for feature, hidden_log in zip(features, hidden_logs):
        ...
        impact = abs(delta_log)
        current_value = current_input.get(feature, np.nan)
        rows.append(
            {
                "feature": feature,
                "current_value": None if _is_missing_value(current_value) else current_value,
                "pred_log_without_feature": float(hidden_log),
                "delta_log": float(delta_log),
                "impact_log_abs": float(impact),
    ...
    if top_total > 0:
        top_df["share_top_percent"] = top_df["impact_log_abs"] / top_total * 100.0
    else:
        top_df["share_top_percent"] = 0.0

    if all_total > 0:
        top_df["share_all_percent"] = top_df["impact_log_abs"] / all_total * 100.0
        coverage = top_total / all_total * 100.0
    else:
        top_df["share_all_percent"] = 0.0
        coverage = 0.0
